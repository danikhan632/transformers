...
def create_sinusoidal_positions(config: Qwen3Config, num_pos: int, dim: int) -> jnp.ndarray:
    inv_freq = 1.0 / (config.rope_theta ** (jnp.arange(0, dim, 2)[: dim // 2] / dim))
...
class FlaxQwen3RotaryEmbedding(nn.Module):
    ...
    def setup(self):
        ...
        self.sincos = create_sinusoidal_positions(self.config, self.config.max_position_embeddings, head_dim)
...
class FlaxQwen3Attention(nn.Module):
    ...
    def setup(self):
        ...
        if self.sliding_window is not None:
            idx = jnp.arange(config.max_position_embeddings)
            mask = (idx[:, None] - idx[None, :]) < self.sliding_window
            mask = jnp.triu(mask)
            self.sliding_causal_mask = mask[None, None, :, :]
        self.rotary_emb = FlaxQwen3RotaryEmbedding(config, dtype=self.dtype)
    ...
    def __call__(
        ...
        if self.has_variable("cache", "cached_key"):
            ...
            if self.sliding_window is not None:
                sliding_mask = lax.dynamic_slice(
                    self.sliding_causal_mask,
                    (0, 0, mask_shift, 0),
                    (1, 1, query_length, max_decoder_length),
                )
                causal_mask = combine_masks(causal_mask, sliding_mask)
        else:
            causal_mask = self.causal_mask[:, :, :query_length, :key_length]
            if self.sliding_window is not None:
                causal_mask = combine_masks(causal_mask, self.sliding_causal_mask[:, :, :query_length, :key_length])
...
class FlaxQwen3DecoderLayer(nn.Module):
    ...
    def setup(self):
        ...
        self.mlp = FlaxQwen3MLP(self.config, dtype=self.dtype)
...
class FlaxQwen3LayerCollection(nn.Module):
    ...
    def setup(self):
        self.blocks = [
            FlaxQwen3DecoderLayer(self.config, i, dtype=self.dtype, name=str(i))
...
__all__ = ["FlaxQwen3ForCausalLM", "FlaxQwen3Model", "FlaxQwen3PreTrainedModel"]
